...
private slots:
    ...
    void whatTheManuscriptIsGoesIntoTheHeader()
    {
        // The keys the published repository's manifest generator reads, so a
        // transcription added to the library fills the download window's
        // columns without being described a second time by hand.
        CombinedDraft draft;
        draft.reference.id = QStringLiteral("Rev.1.1");
        draft.reference.book = QStringLiteral("Rev");
        draft.reference.chapter = 1;
        ...
        const QString xml =
            serializeCombinedOsis({{draft.reference.id, draft}}, metadata);
        QVERIFY(xml.contains(
            QStringLiteral("<description type=\"x-folios\">1r–4v</description>")));
        QVERIFY(xml.contains(QStringLiteral(
            "<description type=\"x-translated-from\">Translated from the Greek"
            "</description>")));
        // One nobody answered is not written at all, rather than written empty.
        QVERIFY(!xml.contains(QStringLiteral("x-material")));
    }

    void theDescriptionsStandBeforeTheType()
    {
        // A work's children are a schema sequence: description comes before
        // type, and a validator is the only thing that would ever say so.
        CombinedDraft draft;
        draft.reference.id = QStringLiteral("Rev.1.1");
        draft.reference.book = QStringLiteral("Rev");
        draft.reference.chapter = 1;
        ...
        const QString xml =
            serializeCombinedOsis({{draft.reference.id, draft}}, metadata);
        QVERIFY(
            xml.indexOf(QStringLiteral("type=\"x-exemplar\""))
            < xml.indexOf(QStringLiteral("<type type=")));
        // And after the plain description Milah writes about itself, which is
        // the other half of being in sequence.
        QVERIFY(
            xml.indexOf(QStringLiteral("<description>"))
            < xml.indexOf(QStringLiteral("type=\"x-exemplar\"")));
    }

    void aWorkWithNoDescriptionsIsWrittenAsItAlwaysWas()
    {
        CombinedDraft draft;
        draft.reference.id = QStringLiteral("Rev.1.1");
        draft.reference.book = QStringLiteral("Rev");
        ...
        const QString xml = serializeCombinedOsis({{draft.reference.id, draft}});
        QVERIFY(!xml.contains(QStringLiteral("<description type=")));
        QVERIFY(xml.contains(QStringLiteral(
